...
for(const src of ENGINE_SOURCES){
  try{ THREE=await import(src); if(THREE?.Scene) break; }catch(err){ console.warn('Skybound renderer source failed',src,err); }
}
if(!THREE?.Scene){
  const st=document.getElementById('status');
  if(st) st.textContent='3D renderer could not load — refresh once or check content blockers';
  document.body.classList.add('renderer-failed');
  throw new Error('Skybound: all Three.js renderer sources failed');
}
...
async function getLoader(){
  ...
  const sources=[
    ...
    'https://unpkg.com/three@0.180.0/examples/jsm/loaders/GLTFLoader.js?module'
  ];
  let lastErr=null;
  for(const src of sources){
    try{ const mod=await import(src); if(mod?.GLTFLoader){loader=new mod.GLTFLoader();return loader;} }catch(err){lastErr=err;console.warn('GLTF loader source failed',src,err)}
  }
  throw lastErr||new Error('GLTFLoader unavailable');
}
...
const body=new THREE.Mesh(new THREE.CapsuleGeometry(2.2,28,8,18),fbWhite); body.rotation.x=Math.PI/2; fallbackPlane.add(body);
